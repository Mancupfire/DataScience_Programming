...
def recursive_forecast(model, initial_data, horizon_hours, target_col='PM2.5', base_features=None):
    ...
    for i in range(horizon_hours):
        ...
        # We need to ensure columns match the model's expected input
        # XGBoost is sensitive to column order/names if passed as DMatrix or dataframe
        # We'll assume the model was trained on the columns generated by create_temporal_features
        ...
        # Ideally, we'd get the feature names from the model, but XGBoost python API 
        # makes that a bit tricky depending on version. 
        # We will rely on the feature engineering being identical.
